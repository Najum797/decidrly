/* ============================================================
   ...
   Sections:
     1.  Fmt          — number / currency formatting (BVR style)
     2.  Comma inputs — attachCommaFormat, initCommaInputs
     3.  Slider sync  — initSliders
     ...
     8.  CalcCharts   — BVR chart defaults + helpers
     9.  ShareURL     — encode/decode URL params
     10. ExportHelper — copy text, print PDF
     11. Tooltips     — hover tooltip engine
     12. Accordion    — initAccordions
     13. DOM utils    — $, setText, setHTML, addClass, removeClass
     14. CURRENCIES   — SE/Freelance currency config
     15. fmt / fmtShort / currencySymbol  — SE/Freelance formatters
     16. randn / randNormal               — Box-Muller RNG
     17. runSalaryErosion                 — SE Monte Carlo engine
     18. buildFanChart / buildHistogram   — SE chart builders
     19. runFreelancerIncome              — Freelance Monte Carlo engine
     20. buildMonthlyFanChart / buildDonutChart  — Freelance charts
...
const CityPresets = {
  ...
  apply(marketKey, cityName) {
    ...
    const sliders = { downPct:city.downPct, mortRate:city.mortRate, loanYears:city.loanYears, rentGrowth:city.rentGrowth, propTax:city.propTax, appreciation:city.appreciation, investReturn:city.investReturn, maintenance:city.maintenance, discount:city.discount || 3 };
  ...
  defaults: {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        cornerRadius: 8, borderColor: 'rgba(255,255,255,0.08)', borderWidth: 1
      }
    },
    scales: {
...
const Tooltips = {
  definitions: {
    ...
    discount:     'Used to convert future costs to today\'s money (inflation adjustment).',
    homePrice:    'The total purchase price of the property.',
    downPct:      'The percentage of the home price you pay upfront.',
    loanYears:    'How long your mortgage runs. Longer term = lower monthly payments but more total interest.',
    mortRate:     'The annual interest rate on your mortgage.',
    monthlyRent:  'What you\'d pay per month in rent for an equivalent property.',
    rentGrowth:   'How much rent increases each year.',
    propTax:      'Annual property tax as a percentage of the home\'s current value.',
    maintenance:  'Annual cost of repairs and upkeep as a percentage of home value.',
    hoa:          'Monthly fees paid to a homeowners association (if applicable).',
    insurance:    'Annual home insurance premium.',
    investReturn: 'Annual return you\'d earn by investing the down payment instead of buying.',
    appreciation: 'How much the home\'s value grows per year.'
  },
  init() {
...
function runSalaryErosion(p) {
  ...
  function p50AtRaise(tr) {
    ...
    for (let r = 0; r < 1000; r++) {
      ...
      for (let y = 1; y <= years; y++) {
        if (raiseFrequency > 0 && y % raiseFrequency === 0)
          nom *= (1 + Math.max(0, randNormal(tr, raiseStd)));
  ...
  for (let i = 0; i < 20; i++) {
    const mid = (lo + hi) / 2;
    if (p50AtRaise(mid) >= startReal) { beRaise = mid; hi = mid; } else lo = mid;
  }
...
/* Shared chart options for SE + Freelance (light theme) */
const _seOpts = {
  ...
  plugins: {
    legend: { display: false },
    tooltip: {
      backgroundColor: '#1A1A2E', borderColor: 'rgba(255,255,255,0.08)', borderWidth: 1,
      titleColor: '#9AA3B0', bodyColor: '#e8edf5', padding: 10
    }
  },
  scales: {
...
function buildDonutChart(canvasId, data) {
  ...
  _charts[canvasId] = new Chart(ctx, {
    ...
    options: {
      responsive: true, maintainAspectRatio: true, cutout: '62%', animation: { duration: 500 },
      plugins: { legend: { display: false }, tooltip: { backgroundColor: '#1A1A2E', borderColor: 'rgba(255,255,255,0.08)', borderWidth: 1, titleColor: '#9AA3B0', bodyColor: '#e8edf5', callbacks: { label: ctx => ` ${ctx.label}: ${fmt(ctx.parsed, currencyCode)}` } } }
    }
  });
}

/* ── 21. SE/FREELANCE BINDERS ────────────────────────────── */
...
/* ── 22. PLATFORM FEES (already defined in section 19) ─── */
